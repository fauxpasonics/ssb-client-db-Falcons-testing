...
/****************************************************
UPDATE LOG
3/19/18 - Updated customer_matchkey (was previously NULL for all records)



****************************************************/
CREATE VIEW [ods].[vw_LoadDimCustomer_IBM_Email] AS ( --Change "SOURCESYSTEM" to the correct source
...
--base set
		SELECT
			...
			, 3 AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules
			...
			/*AddressPrimary*/
            , CASE WHEN ISNULL(z.Address1_Line2,'') != '' AND ISNULL(z.Address1_Line3,'') != '' THEN 
					z.Address1_Line1 + ' ' + z.Address1_Line2 + ' ' + z.Address1_Line3
					WHEN ISNULL(z.Address1_Line2,'') != '' AND ISNULL(z.Address1_Line3,'') = '' THEN 
					z.Address1_Line1+ ' ' + z.Address1_Line2
					WHEN ISNULL(z.Address1_Line2,'') = '' AND ISNULL(z.Address1_Line3,'') != '' THEN
					z.Address1_Line1 + ' ' + z.Address1_Line3
					WHEN ISNULL(z.Address1_Line2,'') = '' AND ISNULL(z.Address1_Line3,'') ='' THEN
					z.Address1_Line1 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street eg. CASE WHEN ISNULL(address2,'') <> '' THEN z.street + ' ' + z.address2 ELSE z.street END AS [AddressPrimaryStreet]
			, NULL AS [AddressPrimarySuite] -- ALWAYS LEAVE THIS NULL!!!!!!!!!!!!!!!!
			, z.Address1_City AS [AddressPrimaryCity] 
			, z.Address1_StateOrProvince AS [AddressPrimaryState] 
			, z.Address1_PostalCode AS [AddressPrimaryZip] 
			, z.Address1_County AS [AddressPrimaryCounty]
			, z.Address1_Country	AS [AddressPrimaryCountry] 
			...
			/*Phone*/
			, LEFT(z.Telephone1,25) AS [PhonePrimary] -- For Coalescing values make sute they are wrapped in a NULLIF for values that = '' eg COALESCE(NULLIF(z.mobile,''),z.phone) AS [PhonePrimary]
			, LEFT(z.Telephone2,25) AS [PhoneHome]
			, LEFT(z.MobilePhone,25) AS [PhoneCell]
			, LEFT(z.Telephone1,25) AS [PhoneBusiness]
			, LEFT(NULL,25) AS [PhoneFax]
			, LEFT(z.[Web Form Telephone1],25) AS [PhoneOther]
			...
			, z.[Opt In Date] [SSCreatedDate] --createddate
			, z.[Last Modified Date] [SSUpdatedDate] --lastmodifiedbyid
			...
			, 'IMC-' +  CAST(z.RECIPIENT_ID AS NVARCHAR(200)) AS [Customer_Matchkey] /*Standard for macthkeys are ‘TM-xxxx’, ‘Pac-xxxx’ depending on source, 
		...
		--SELECT COUNT(*)			
		FROM [ods].IMC_ProductionCopy_MasterDB z WITH (NOLOCK) --Change To Source Table
		WHERE 1=1
