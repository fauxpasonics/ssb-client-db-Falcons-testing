...
CREATE VIEW [etl].[vw_Load_DimCustomer_CRM_Contact_365] AS 
...
--base set
		...
		SELECT 'Falcons' AS [SourceDB]
		, 'CRM_Contact' AS [SourceSystem]
		, 4 AS [SourceSystemPriority]
		...
		/*AddressPrimary*/
			...
			, '365-' + CAST(c.parentcustomerid AS NVARCHAR(50)) AS [ExtAttribute2] 
			...
			, CAST(c.kore_primaryarchticsid AS INT) [AccountId]
			, 0 AS IsBusiness
			, CASE WHEN ISNULL(c.new_ssbcrmsystemssidwinner,'') = '' OR ISNULL(c.new_ssbssidwinnersourcesystem,'') = '' THEN NULL
					WHEN c.new_ssbssidwinnersourcesystem = 'TM' THEN 'TM-' + new_ssbcrmsystemssidwinner
					WHEN c.new_ssbssidwinnersourcesystem = 'Fanatics' THEN 'FAN-' + new_ssbcrmsystemssidwinner
					WHEN c.new_ssbssidwinnersourcesystem = 'IBM_Email' THEN 'IMC-' + new_ssbcrmsystemssidwinner
					WHEN c.new_ssbssidwinnersourcesystem = 'Legacy_Dynamics_Contact' THEN 'LCRMC-' + new_ssbcrmsystemssidwinner
					WHEN c.new_ssbssidwinnersourcesystem = 'TM-Falcons_Dome' THEN 'TMD-' + new_ssbcrmsystemssidwinner
					WHEN c.new_ssbssidwinnersourcesystem = 'TM-Falcons_PSL' THEN 'TMPSL-' + new_ssbcrmsystemssidwinner
					WHEN c.new_ssbssidwinnersourcesystem = 'TM-Falcons_Dome' THEN 'TMD-' + new_ssbcrmsystemssidwinner
					WHEN c.new_ssbssidwinnersourcesystem = 'TM-Falcons_Dome' THEN 'TMD-' + new_ssbcrmsystemssidwinner
					WHEN c.new_ssbssidwinnersourcesystem = 'TM-Falcons_Dome' THEN 'TMD-' + new_ssbcrmsystemssidwinner
					WHEN c.new_ssbssidwinnersourcesystem = 'Legacy_Dynamics_Account' THEN 'LCRMA-' + new_ssbcrmsystemssidwinner
					WHEN c.new_ssbssidwinnersourcesystem = 'FB_Leads' THEN 'FB-' + new_ssbcrmsystemssidwinner
					WHEN c.new_ssbssidwinnersourcesystem = 'FanCentric' THEN 'FC-' + new_ssbcrmsystemssidwinner
					ELSE NULL END AS customer_matchkey

			FROM Falcons_reporting.Prodcopy.Contact c
			WHERE (c.ModifiedOn > DATEADD(DAY,-7,GETDATE()) OR c.CopyLoadDate > DATEADD(DAY,-7,GETDATE()))
			AND c.modifiedon > '2018-04-11 05:10:29.000'
