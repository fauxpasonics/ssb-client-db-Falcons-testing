...
/*********************************
CHANGE LOG 
3/19/18 - TCF - Updated customer_matchkey (previously all null)


*********************************/
CREATE VIEW [etl].[vw_Load_DimCustomer_FB_Leads] AS 
...
--base set
		SELECT
			...
			, 'FB_Leads' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, 1 AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(REPLACE(z.id,'l:','') AS NVARCHAR(200)) [SSID] -- Required
			...
			, REPLACE(z.zip_code,'z:','') AS [AddressPrimaryZip] 
			...
			/*Phone*/
			, LEFT(CASE WHEN z.phone_number LIKE 'p:[0-9]%' THEN REPLACE(z.phone_number,'p:','') WHEN z.phone_number
			LIKE 'p:+[0-9]%' THEN REPLACE(z.phone_number,'p:+','') END,25) AS [PhonePrimary]
			...
			/*Extended Attributes*/
			, z.ad_id AS[ExtAttribute1] --nvarchar(100)
			, NULL AS[ExtAttribute2] --nvarchar(100)
			, z.campaign_id AS[ExtAttribute3] --nvarchar(100)
			, z.form_id AS[ExtAttribute4] --nvarchar(100)
			, z.adset_id AS[ExtAttribute5] --nvarchar(100)
			...
			, z.adset_name AS[ExtAttribute10] --nvarchar(1000)
			...
			, CAST(REPLACE(REPLACE(z.created_time,'T',' '),'-04:00','') AS DATETIME) [SSCreatedDate] --createddate
			...
			, 0 AS IsBusiness
			, 0 AS isDeleted
			, CASE WHEN ISNULL(z.id,'') != '' THEN  'FB-' + CAST(REPLACE(z.id,'l:','') AS NVARCHAR(200)) ELSE NULL END AS [Customer_Matchkey] /*Standard for macthkeys are ‘TM-xxxx’, ‘Pac-xxxx’ depending on source, 
											Customer Matchkey needs to have fairly complicated logic. 
											See Writup for details.*/
		--SELECT COUNT(*)			
		FROM ods.Facebook_Leads z WITH (NOLOCK) --Change To Source Table
		WHERE 1=1
		AND DATEDIFF(DAY,z.etl_updateddate,GETDATE())<=10 --OR DATEDIFF(DAY,z.added,GETDATE())<=10 --This needs to be uncommented out after the first load, update the date fields
	) a
