...
DCH on 2018-03-20:	intended to load Falcons.DimCustomer from Falcons_PSL.
...
CREATE VIEW [ods].[vw_TM_LoadDimCustomer_PSL] AS (
...
--base set
		SELECT 
			'Falcons_PSL' AS [SourceDB]
			, (Falcons_PSL.etl.fnGetClientSetting('TM-SourceStyem')) AS [SourceSystem]
			...
			, 'TMPSL-'+ CAST(acct_id AS NVARCHAR(100)) + ':' + CAST(cust_name_id AS NVARCHAR(100)) AS [Customer_Matchkey]
			
		FROM Falcons_PSL.ods.TM_Cust
		WHERE 1=1
