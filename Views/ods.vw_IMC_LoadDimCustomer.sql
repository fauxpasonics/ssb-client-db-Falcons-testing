...
CREATE VIEW [ods].[vw_IMC_LoadDimCustomer] AS ( --Change "Source" to the correct source
...
--base set
		SELECT
			'Falcons' AS [SourceDB] --Client Source Database such as Raiders or Raiders_Reporting
			, 'IBM Marketing Cloud' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(imc.RECIPIENT_ID AS NVARCHAR(50)) [SSID] -- Required
			...
			, CAST(imc.BirthDate AS DATETIME) AS [Birthday] -- field is sensitive see writup in sharepoint for an example of how complicated it can get
			, imc.GenderCode AS [Gender] 
			, 0 [MergedRecordFlag]
			, NULL [MergedIntoSSID]
			...
			/*Name*/
			, NULL AS [Prefix]
			, imc.FirstName AS [FirstName]
			, imc.MiddleName AS [MiddleName]
			, imc.LastName AS [LastName]
			, NULL AS [Suffix]
			, NULL AS [Fullname]
			--, c.name_title as [Title]

			/*AddressPrimary*/
            , CASE WHEN ISNULL(imc.Address1_Line2,'')<>'' THEN imc.Address1_Line1+' '+imc.Address1_Line2 WHEN ISNULL(imc.Address1_Line2,'')=''
			THEN imc.Address1_Line1 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
			, imc.Address1_City AS [AddressPrimaryCity] 
			, imc.Address1_StateOrProvince	AS [AddressPrimaryState] 
			, imc.Address1_PostalCode AS [AddressPrimaryZip] 
			, imc.Address1_County AS [AddressPrimaryCounty]
			, imc.Address1_Country AS [AddressPrimaryCountry] 
			...
			/*Phone*/
			, CASE WHEN ISNULL(imc.Telephone1,'')<>'' THEN LEFT(imc.Telephone1,25)
				WHEN ISNULL(imc.Telephone1,'')='' AND ISNULL(imc.Telephone2,'')<>'' THEN LEFT(imc.Telephone2,25)
				WHEN ISNULL(imc.Telephone1,'')='' AND ISNULL(imc.Telephone2,'')='' AND ISNULL(imc.[Web Form Telephone1],'')<>'' THEN LEFT(imc.[Web Form Telephone1],25)
				WHEN ISNULL(imc.Telephone1,'')='' AND ISNULL(imc.Telephone2,'')='' AND ISNULL(imc.[Web Form Telephone1],'')='' AND ISNULL(imc.MobilePhone,'')<>'' THEN LEFT(imc.MobilePhone,25) ELSE '' END AS [PhonePrimary]
			, LEFT(NULL,25) AS [PhoneHome]
			, LEFT(imc.MobilePhone,25) AS [PhoneCell]
			, LEFT(NULL,25) AS [PhoneBusiness]
			, LEFT(NULL,25) AS [PhoneFax]
			, LEFT(NULL,25) AS [PhoneOther]

			/*Email*/
			, CAST(imc.Email AS NVARCHAR(256)) AS [EmailPrimary]
			...
			, imc.[Opt In Date] SSCreatedDate  
			, imc.[Last Modified Date] SSUpdatedDate
			, GETDATE()	CreatedDate
			, GETDATE()	UpdatedDate
			, 0 AS IsDeleted
			, NULL [AccountId]
...
--		select top 100 *			
		FROM ods.IMC_ProductionCopy_MasterDB imc WITH (NOLOCK) 
		WHERE 1=1
