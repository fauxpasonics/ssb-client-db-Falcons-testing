...
--base set
		...
		/*AddressPrimary*/
			...
			, 0 AS IsBusiness
			--, CASE WHEN new_TMAccountID IS NOT NULL THEN CONCAT('TM-',new_TMAccountID, 
			--CASE WHEN new_TMcustomernameid IS NOT NULL THEN CONCAT(':', new_TMcustomernameid) ELSE ''END) ELSE NULL END AS customer_matchkey	
			--select count(*)
			FROM Prodcopy.vw_Contact c WITH (NOLOCK)
