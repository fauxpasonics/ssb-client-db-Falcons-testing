...
/****************************************************
UPDATE LOG
3/19/18 (TCF) - Updated customer_matchkey (was previously NULL for all records)
			Repointed view to reporting.prodcopy.account instead of vw_account
			Removed deleteddate from query
...
create VIEW [etl].[vw_Load_DimCustomer_CRM_Account_365] AS 
...
--base set
		...
		/*AddressPrimary*/
			...
			FROM falcons_reporting.Prodcopy.Account c
			WHERE c.ModifiedOn > DATEADD(DAY,-3,GETDATE()) OR c.CopyLoadDate > DATEADD(DAY,-3,GETDATE()) 
