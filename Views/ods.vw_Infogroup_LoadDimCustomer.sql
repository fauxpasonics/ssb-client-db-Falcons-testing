...
CREATE VIEW [ods].[vw_Infogroup_LoadDimCustomer] AS ( --Change "Source" to the correct source
...
--base set
		SELECT
			...
			/*Standard Attributes*/
			, CAST(z.ETL_ID AS NVARCHAR(50)) [SSID] -- Required
			...
			, z.CE_Individual_HoH_Gender AS [Gender] 
			, 0 [MergedRecordFlag]
			, NULL [MergedIntoSSID]
			...
			/*Name*/
			, NULL AS [Prefix]
			, ISNULL(z.CE_Individual_HoH_Given_Name,'') AS [FirstName]
			, ISNULL(z.CE_Individual_HoH_Middle_Initial,'') AS [MiddleName]
			, ISNULL(z.CE_Individual_HoH_Surname,'') AS [LastName]
			, NULL AS [Suffix]
			, NULL AS [Fullname]
...
--		SELECT top 100 *			
		FROM ods.Infogroup_Demographic z WITH (NOLOCK) 
		WHERE 1=1
		AND ETL_UpdatedDate > DATEADD(DAY,-3,GETDATE()) --This needs to be uncommented out after the first load, update the date fields
