...
CREATE VIEW [apietl].[vw_QuestionResponse]
AS 
SELECT
	...
	am.scaleValue,
	am.preCode
FROM apietl.vw_VisionCritical_AnswerMap am
INNER JOIN (
		SELECT DISTINCT
		...
		INNER JOIN apietl.vc_af_records_values_1 responses (NOLOCK)
			ON  respondents.vc_af_records_id = responses.vc_af_records_id
		LEFT OUTER JOIN apietl.vc_af_records_values_value responseValues (NOLOCK)
			ON  responses.vc_af_records_values_id = responseValues.vc_af_records_values_id
	) responses
