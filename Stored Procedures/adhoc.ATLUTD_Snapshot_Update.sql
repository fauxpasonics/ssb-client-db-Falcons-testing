...
CREATE PROCEDURE [adhoc].[ATLUTD_Snapshot_Update]
...
--//////////////Set Initial Parameters//////////////////////
	...
	Set @SeasonYear = '2017'  --change year

	DECLARE @Tablename varchar (50) --increased table name to avoid truncation
	Set @Tablename = '[adhoc].[ATLUTD_Snapshot]'  --change table name
...
--//////////////Develop Loop Logic//////////////////////
	...
	INNER JOIN 
			( SELECT 
			  ...
			  FROM dbo.dimevent where dimseasonid IN( '144', '166')
	...
	WHERE fts.IsSold = 1 AND fts.IsSaleable = 1 AND de.dimseasonid IN ( '144', '166') --changed dimseasonid  from 49 to 72 for the current season 2016 Falcons Season
	AND prerank.nopre >2
	AND de.eventdate >= (select dateadd(hour, UTCOffset, GETDATE()) from dbo.DimDate where CalDate = cast(getdate() as date)) 
	AND de.EventName NOT IN('TBA','')
	Group by fts.dimeventid , de.eventname

	DECLARE @Counter int
	SET @Counter = '1'

	WHILE @Counter <= '20' --change counter to be larger than that of the highest row ranking (above)
	BEGIN
...
--//////////////Set Team and Team with Quotes////////////////
	...
	SET @EventID = (SELECT DimEventId FROM #LoopCounter WHERE ListCount=@Counter)
...
--/////////////Set Daysout by Opponent Event Date and Place Quotes////////////
	...
	WHERE   DimSeasonId IN ( '144','166') AND de.EventName = @TEAM AND @EventID=de.DimEventId --changed dimseasonid  from 49 to 72 for the current season 2016 Falcons Season
	)
...
--///////////Develop Dynamic Query for calulating current available inventory//////////
	...
	WHERE fts.IsSold = 1 AND fts.IsSaleable = 1 AND De.Eventname =  @Team AND c.classname IS NOT NULL	
