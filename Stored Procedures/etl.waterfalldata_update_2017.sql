...
-- =============================================
--TESTING Exec [etl].[waterfalldata_update_2017]
-- select * from etl.waterfalldata2017
CREATE PROCEDURE [etl].[waterfalldata_update_2017]
...
--//////////////Set Team and Team with Quotes////////////////
	...
	--Replaced logic to split name due to naming convention change ZFrick 2016-07-29 
	--SET @TEAM17 = (select RIGHT( eventname,charindex(' ', REVERSE(eventname))-1) from #LoopCounter where ListCount = @Counter)

	SET @TEAM17 = (select eventname from #LoopCounter where ListCount = @Counter)
