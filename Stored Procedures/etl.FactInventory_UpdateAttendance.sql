...
CREATE PROCEDURE [etl].[FactInventory_UpdateAttendance]
...
BEGIN
	...
	INTO #AttendTM
	FROM #AttendData a
	LEFT OUTER JOIN rpt.vw_DimEvent de ON de.SSID_event_id = a.event_id
	LEFT OUTER JOIN rpt.vw_DimSeat dst ON dst.SSID_manifest_id = de.ManifestId AND dst.SSID_section_id = a.section_id AND dst.SSID_row_id = a.row_id AND dst.SSID_seat = a.seat_num AND dst.SourceSystem = 'TM'
