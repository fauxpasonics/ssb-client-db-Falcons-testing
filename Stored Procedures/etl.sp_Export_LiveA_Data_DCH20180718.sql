...
CREATE PROCEDURE [etl].[sp_Export_LiveA_Data_DCH20180718]
AS
...
/*					doesn't appear to be used in result set or WHERE clause	DCH 2018-07-18
LEFT JOIN ods.LiveAnalytics_Customer la (NOLOCK)	
...
--	clear out tempdb
DROP TABLE #tmp_vwCompositeRecord_ModAcctID;
...
/*			doesn't appear to be used in result set or WHERE clause	DCH 2018-07-18
	LEFT JOIN dbo.DimCustomer T6
    ON T6.AccountId = T5.assoc_acct_id
		AND T6.SourceSystem = 'TM'
		AND T6.CustomerType = 'Primary'
		AND T6.IsDeleted = 0
*/
WHERE 1 = 1
AND (t4.PC2 IS NULL OR t4.PC2 <> 'C')
--	AND T2.EventDate >= '2017-03-01'		moved above to join condition	DCH 2018-07-18
--	and T5.activity_name is not null		already commented out
...
JOIN dbo.vwCompositeRecord_ModAcctID compr (NOLOCK)
	ON compr.SSB_CRMSYSTEM_CONTACT_ID = t2.SSB_CRMSYSTEM_CONTACT_ID
LEFT JOIN ods.LiveAnalytics_Customer la (NOLOCK)	
        ON la.cust_first_nm = compr.FirstName
           AND la.cust_last_nm = compr.LastName
           AND la.cust_addr_line_1 = compr.AddressPrimaryStreet
           AND la.cust_email_addr = compr.EmailPrimary
WHERE compr.EmailPrimary NOT LIKE '%@falcons.nfl.net'
		AND compr.EmailPrimary NOT LIKE '%@ambfo.com'
		AND compr.EmailPrimary NOT LIKE '%@ambse.com'
		AND compr.EmailPrimary NOT LIKE '%@mercedesbenzstadium.com'
		AND compr.EmailPrimary NOT LIKE '%@atlutd.com'
