...
CREATE PROCEDURE [rpt].[Cust_SelloutStrategy_C] 
(
...
FROM (
	...
	SELECT 'Unsold' SaleStatus, f.DimEventId, dst.Config_Location, ISNULL(c.ClassName, 'Unclassified') TicketType
	, COUNT(*) Qty
	...
	GROUP BY f.DimEventId, dst.Config_Location, ISNULL(c.ClassName, 'Unclassified')
) a
...
ORDER BY
    ro.EventDate, ro.Section1, ro.Section2, tts.SortOrder;
