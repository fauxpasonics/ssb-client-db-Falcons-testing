...
GO
	...
	CREATE TRIGGER [dbo].[FirstName_EmailPrimary]
	ON [dbo].[DimCustomer]
	AFTER UPDATE
	AS 
	BEGIN
	IF UPDATE(FirstName) OR UPDATE (EmailPrimary)
		BEGIN
...
GO
	...
	CREATE TRIGGER [dbo].[FuzzyName_PhonePrimary]
	ON [dbo].[DimCustomer]
	AFTER UPDATE
	AS 
	BEGIN
	IF UPDATE(FuzzyNameGUID) OR UPDATE (PhonePrimary)
		BEGIN
		UPDATE dbo.DimCustomer
		SET matchkey_updatedate = CURRENT_TIMESTAMP
		FROM dbo.DimCustomer a
...
GO
	...
	CREATE TRIGGER [dbo].[Name_AccountID]
	ON [dbo].[DimCustomer]
	AFTER UPDATE
	AS 
	BEGIN
...
GO
CREATE NONCLUSTERED INDEX [IDX_AccountId] ON [dbo].[DimCustomer] ([AccountId]) INCLUDE ([CustomerType], [SourceSystem])
GO
...
GO
CREATE NONCLUSTERED INDEX [NCIX_DimCustomer_SourceSystem_IsDeleted_Include] ON [dbo].[DimCustomer] ([SourceSystem], [IsDeleted]) INCLUDE ([DeleteDate], [SSID], [UpdatedDate])
GO
